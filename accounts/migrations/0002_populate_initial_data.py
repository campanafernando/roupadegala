...
def load_cities_from_sql(apps, schema_editor):
    ...
    cities_sql_path = os.path.join(settings.BASE_DIR, "cities.sql")
    ...
    with connection.cursor() as cursor:
        cursor.execute(f"SELECT COUNT(*) FROM {City._meta.db_table}")  # Usa o nome real da tabela
...
def insert_person_types(apps, schema_editor):
    ...
    """
    PersonType = apps.get_model("accounts", "PersonType")  # Garante que a tabela já existe
    existing_types = {pt.type for pt in PersonType.objects.all()}  # Obtém os tipos já existentes
